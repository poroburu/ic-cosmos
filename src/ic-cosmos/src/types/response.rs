...
pub struct RpcVersionInfo {
    /// The current version of cosmos-core
    #[serde(rename = "cosmos-core")]
    pub cosmos_core: String,
    /// first 4 bytes of the FeatureSet identifier
...
impl fmt::Debug for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.cosmos_core)
    }
}

impl fmt::Display for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Some(version) = self.cosmos_core.split_whitespace().next() {
