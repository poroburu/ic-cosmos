...
pub fn encode_metrics(w: &mut ic_metrics_encoder::MetricsEncoder<Vec<u8>>) -> std::io::Result<()> {
    METRICS.with(|m| {
        ...
        w.encode_gauge(
            "cos_canister_version",
            ic_cdk::api::canister_version().metric_value(),
            "Canister version",
        )?;
        w.encode_gauge(
            "cos_stable_memory_pages",
        ...
        w.counter_entries("cos_requests", &m.requests, "Number of JSON-RPC requests");
        w.counter_entries("cos_responses", &m.responses, "Number of JSON-RPC responses");
        w.counter_entries(
            "cos_inconsistent_responses",
            &m.inconsistent_responses,
            "Number of inconsistent RPC responses",
        );
        w.counter_entries(
            "cos_err_http_outcall",
            &m.err_http_outcall,
            "Number of unsuccessful HTTP outcalls",
        );
        w.counter_entries(
            "cos_err_unauthorized",
            &m.err_unauthorized,
            "Number of unauthorized errors for canister methods",
        );
        w.counter_entries(
            "cos_err_host_not_allowed",
            &m.err_host_not_allowed,
            "Number of HostNotAllowed errors",
        );
        w.encode_counter(
            "cos_err_no_permission",
