...
use ic_cosmos_wallet::state::InitArgs;
use ic_test_utilities_load_wasm::load_wasm;
use serde::de::DeserializeOwned;
use test_utils::{CallFlow, TestSetup};

thread_local! {
     static RPC_WASM: Vec<u8> = load_wasm(get_root(), "ic-cosmos-rpc", &[]);
     static WASM: Vec<u8> = load_wasm(env!("CARGO_MANIFEST_DIR"), env!("CARGO_PKG_NAME"), &[]);
}

#[derive(Clone)]
pub struct CosmosWalletSetup {
    rpc_setup: TestSetup,
    setup: TestSetup,
}

impl Default for CosmosWalletSetup {
    fn default() -> Self {
        Self::new()
    }
}

impl CosmosWalletSetup {
    pub fn new() -> Self {
        let rpc_setup = TestSetup::new(
            RPC_WASM.with(|wasm| wasm.clone()),
            ic_cosmos_rpc::state::InitArgs {
                demo: Some(true),
                managers: Some(vec![TestSetup::controller_id()]),
            },
        );

        let cos_canister = Some(rpc_setup.canister_id);
